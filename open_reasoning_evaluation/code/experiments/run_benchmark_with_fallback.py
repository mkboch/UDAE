...
def main():
    ...
    try:
        ...
        for strategy in args.strategies:
            print(f"\n--- model={args.model} dataset={args.dataset} strategy={strategy} n={len(dataset_rows)} ---")
            run_rows = []
            for i, rec in enumerate(dataset_rows, start=1):
                ...
                prompt = maybe_apply_chat_template(
                    tokenizer,
                    prompt,
                    use_chat_template=bool(chosen_cfg.get("use_chat_template", True)),
                )
            ...
            out_path = RAW_DIR / f"{args.tag}__{args.model}__{args.dataset}__{strategy}__n{len(run_rows)}.csv"
            write_rows_csv(out_path, run_rows)
            print(f"Saved: {out_path}")

        combined = RAW_DIR / f"{args.tag}__{args.model}__ALL_RUNS__n{len(all_rows)}.csv"
